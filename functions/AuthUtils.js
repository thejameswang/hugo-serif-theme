...
// three hours, forces a call to auth0
const NETLIFY_JWT_EXPIRATION_SECONDS = 3 * 3600;
...
const claimNamespace = process.env.AUTH0_TOKEN_NAMESPACE || "https://netlify-integration.com"
...
const generateNetlifyJWT = async (tokenData) => {
    ...
    const tokenPayload = {
        ...
        app_metadata: {
            authorization: {
                roles: tokenData[claimNamespace + '/roles'],
            },
        },
    };
...
const generateNetlifyCookieFromAuth0Token = async (tokenData) => {
    ...
    console.log("token", netlifyToken);
...
const handleLogin = async (event) => {
    ...
    const referer = parseURL(event.queryStringParameters.site).href;
    console.log(referer)
